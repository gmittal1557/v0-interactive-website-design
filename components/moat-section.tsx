...
const threats = [
  {
    ...
    level: "High",
    levelColor: "bg-destructive/10 text-destructive/80",
    defense: "Own the cross-district data asset before they reach scale.",
  },
  {
    name: "Google Classroom",
    level: "Medium",
    levelColor: "bg-warm/10 text-warm",
...
export function MoatSection() {
  return (
    <SectionWrapper id="moat" className="py-28" fullHeight={false}>
      <FadeIn>
        <div className="flex items-center gap-4 mb-16">
          <span className="text-xs font-mono text-primary font-semibold tracking-wide">06</span>
          <div className="h-px flex-1 bg-border/50" />
          <span className="text-[11px] font-mono text-muted-foreground/60 uppercase tracking-widest">Why Glean Wins</span>
        </div>
      </FadeIn>
      ...
      <div className="grid lg:grid-cols-2 gap-10 mb-14">
        <div>
          <FadeIn>
            <h2 className="text-3xl md:text-4xl font-serif text-foreground mb-8 text-balance leading-[1.15]">
              Google knows submissions.{" "}
              <span className="text-primary">Glean knows what the district knows.</span>
            </h2>
          </FadeIn>
            ...
            {moats.map((moat, i) => {
              ...
              return (
                <FadeIn key={moat.title} delay={0.08 + i * 0.08}>
                  <div className="p-4 rounded-xl bg-card border border-border/50 group hover:border-primary/15 transition-colors">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-semibold text-foreground mb-0.5">{moat.title}</h4>
        ...
        <div>
          ...
          <FadeIn delay={0.2}>
            <div className="flex items-center gap-2 mb-4">
              <AlertTriangle className="w-3.5 h-3.5 text-warm" />
              <h3 className="text-sm font-semibold text-foreground">Biggest Threats</h3>
            </div>
          </FadeIn>
            ...
            {threats.map((threat, i) => (
              <FadeIn key={threat.name} delay={0.25 + i * 0.07}>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-card border border-border/50">
                  <span className={`text-[10px] font-mono font-semibold px-2 py-0.5 rounded ${threat.levelColor}`}>
                    {threat.level}
                  </span>
